...
class ClaudeInterceptor {
        ...
        try {
            ...
            if (!this.accountInfo?.authorization || this.accountInfo.authorization !== authorization) {
                ...
                if (settingsData) {
                    ...
                    if (this.accountInfo?.emailAddress) {
                        console.log(`[SILENT] [Claude Interceptor] Updating authorization for account: ${this.accountInfo.emailAddress}`);
    ...
    handleTokenExpired(expiredAuthorization) {
        try {
            ...
            if (settingsData) {
                // 查找使用该token的账号
                const account = this.findClaudeAccountByAuthorizationInSettings(settingsData, expiredAuthorization);
                if (account) {
                    console.warn(`[TERMINAL] [Claude Interceptor] Token expired for account: ${account.emailAddress}`);
                    // 清除过期的token
                    account.authorization = undefined;
                    this.updateClaudeAccountAuthorizationInSettings(settingsData, account.emailAddress, undefined);
                    this.saveSettingsToStore(settingsData);
    ...
    async addOrUpdateClaudeAccount(settingsData, account) {
        const serviceProviders = settingsData.serviceProviders || [];
        let claudeProvider = serviceProviders.find(p => p.type === ClaudeInterceptor.PROVIDER_TYPE_CLAUDE_OFFICIAL);
